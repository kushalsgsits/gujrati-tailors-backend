...
public class OrderRepositoryEventHandler {
  ...
  public void beforeCreate(Order order) {
    ...
    // and hence this request will be considered as CREATE request rather than SAVE
    // request

    if (Objects.nonNull(order.getId())) {
      // This is Case 2 i.e order does not exist for given orderId
      // Only thing that needs to be checked is that given orderId should match with
      // computed
      // orderId from payload
  ...
  private void validateOrderId(Order order) {
    ...
    if (!Objects.equals(computedId, order.getId())) {
      throw new IllegalArgumentException(
          String.format(
              "orderId(=%s) from path param and computed orderId(=%s) from payload does not match",
              order.getId(), computedId));
    }
  }

  private String computeId(Order order) {
    return order.getOrderDate().toString().substring(0, "yyyy-MM-".length())
        + order.getOrderType().getShortName()
        + "-"
